...
function Messages() {
  ...
  const messagesByUser = useSelector(
    (state) => state.messageReducer.messagesByUser,
  );
  const loggedInUserId = useSelector(
    (state) => state.messageReducer.loggedInUserId,
  );
  const textFieldForMessages = useSelector(
    (state) => state.messageReducer.textFieldForMessages,
  );
  const secondUserName = useSelector(
    (state) => state.messageReducer.secondUserName,
  );
  const secondUserId = useSelector(
    (state) => state.messageReducer.secondUserId,
  );
  const linearProgress = useSelector(
    (state) => state.messageReducer.linearProgress,
  );
  ...
  useEffect(() => {
    ...
    token &&
      document
        .getElementById("mainContentMessagesBoxPaper")
        .scrollTo(0, 100000);
  }, [dispatch, messagesByUser, token]);
    ...
    <ThemeProvider theme={theme}>
      <div className={classes.root}>
          ...
          <List>
            {users.map((user) => (
              <Paper key={user.id} className={classes.userMessageBoxPaper}>
                ...
                />
                <CardActionArea
                  onClick={() => handleUserMessageBoxClicked(user)}>
                  <h3>{user.name}</h3>
                  <p>{user.message}</p>
                </CardActionArea>
              </Paper>
        ...
        <main
          ...
          <Paper
            id='mainContentMessagesBoxPaper'
            className={classes.mainContentMessagesBoxPaper}>
