...
function SinglePost(props) {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            if (values.bidPrice > (highestBid + 99)) {
                dispatch(Actions.addBid({ postId: post.id, price: values.bidPrice, message: values.bidMessage }));
    ...
    const ratingForm = useFormik({
        ...
        onSubmit: values => {
            console.log({ review: values.rating, postId: post.id, reviewTo: post.user.id });
        },
        validationSchema: Yup.object({
        ...
        <ThemeProvider theme={theme}>
            <Paper className={classes.rootPaper}>
                ...
                <div className={classes.userDiv}>
                    <NavLink style={{ color: '#f50057', textDecoration: 'none' }} to={`/profile/${post.user ? post.user.id : '0'}`}>
                        <h1>
                            {post.user ? post.user.first_name + ' ' + post.user.last_name : ''}
                            (<Rating name="read-only" value={post.user ? post.user.rating : 0} readOnly />)
                        </h1>
                    </NavLink>
                </div>
                <Paper className={classes.postPaper}>
                    <Avatar className={classes.postAvatar} variant="square" alt="No Image" src={`http://127.0.0.1:8000${post.image}`} />
                    ...
                    <div className={classes.bidDetailsSection}>
                        ...
                        {post.bids_for_single_post ? post.bids_for_single_post.map(bid =>
                            <Paper key={bid.id} className={classes.bidMapDiv}>
                                <div className={post.soldTo ? post.soldTo === bid.user.id ? classes.winner : '' : ''}>
                                    <h3 style={{ textAlign: 'center' }}>
                                        {bid.user.first_name + ' ' + bid.user.last_name} {'Rs.' + bid.price}
                                    </h3>
                                    <p>{bid.message}</p>
                                </div>
                                ...
                                <Button
                                    className={post.user ? !((post.user.remember_token === localStorage.getItem('token')) && post.isActive) ? classes.hideDisplayInputBid : '' : ''}
                                    color="secondary"
                ...
                <div style={{ justifyContent: 'center', display: 'flex' }}>
                    {post.bids_for_single_post ?
                        !post.isActive && post.soldTo === loggedInUserId && post.isPaid
                            ...
                            <div>
                                ...
                                <form onSubmit={ratingForm.handleSubmit} className={classes.hideRatingForm}>
                    ...
                    {post.bids_for_single_post ?
                        !post.isActive && post.user.remember_token === localStorage.getItem('token') && post.isPaid
                            ? <Alert variant="filled" severity="success">
